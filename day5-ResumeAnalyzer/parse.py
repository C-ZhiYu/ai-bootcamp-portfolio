...
def read_resume_pdf(path: str) -> str:
    ...
    # 1: Open the PDF file and handle exceptions
    try:
    ...
    # 2: Check for the number of pages and warn if more than 2
    if len(reader.pages) > 2:
        print(f"Warning: Résumé has {len(reader.pages)} pages; ATS systems typically expect 1 page.", file=sys.stderr)

    # 3: Extract text from each page and join with double newlines
    text = "\n\n".join(page.extract_text() or "" for page in reader.pages)

    # 4: Collapse runs of 3 or more consecutive blank lines to 2 + strip leading/trailing whitespace
    text = re.sub(r"\n{3,}", "\n\n", text).strip()

    # 5: Raise error if the text is too short or truncate if too long
...
def read_jd_text(path: str) -> str:
    ...
    """
    # TODO: implement this function
    # 1/2: Open the file and handle exceptions + strip the content
    try:
    ...
    # 3: Raise error if the content is too short
    if len(content) < _MIN_JD_CHARS:
        raise ValueError(f"Job description is too short ({len(content)} chars); likely incomplete.")
